...
class SefatAttendanceSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Clear existing August records for Sefat Raihan
        DB::table('attendance')
        ...
        for ($day = 1; $day <= 31; $day++) {
            ...
            switch ($pattern) {
                case 'late': // Late by 12–28 min
                    $lateMin = ($workingDayIndex % 2 === 0) ? 14 : 26;
                ...
                case 'excessive_late':
                    $inTime  = $date->copy()->setTimeFromTimeString('09:50:00');
                    $outTime = $date->copy()->setTimeFromTimeString('18:00:00');
                    $records[] = $this->makeRecord($employeeId, $shiftId, $inTime, 'Excessive-Late', $outTime, 'On-Time', 'Regular', 50, 0, 0, 'excessive_late');
                    break;

                case 'early_out':
                    $inTime  = $date->copy()->setTimeFromTimeString('09:05:00');
                    $outTime = $date->copy()->setTimeFromTimeString('17:25:00');
                    $records[] = $this->makeRecord($employeeId, $shiftId, $inTime, 'On-Time', $outTime, 'Early-Exit', 'Regular', 0, 35, 0, 'early_out');
                    break;

                case 'late_early_out':
                    $inTime  = $date->copy()->setTimeFromTimeString('09:18:00');
                    $outTime = $date->copy()->setTimeFromTimeString('17:40:00');
                    $records[] = $this->makeRecord($employeeId, $shiftId, $inTime, 'Late', $outTime, 'Early-Exit', 'Regular', 18, 20, 0, 'late');
                    break;

                case 'overtime':
                    $inTime  = $date->copy()->setTimeFromTimeString('09:00:00');
                    $outTime = $date->copy()->setTimeFromTimeString('20:00:00');
                    $records[] = $this->makeRecord($employeeId, $shiftId, $inTime, 'On-Time', $outTime, 'On-Time', 'Regular', 0, 0, 120, 'overtime');
                    break;
                ...
                case 'present':
                default:
                    $inOffset = rand(0, 4); // 0-4 mins after 09:00
        ...
        $this->command->info("✅ Inserted " . count($records) . " attendance records for Sefat Raihan (August 2026)");
