...
@section('content')
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-primary text-white py-3">
                    <h5 class="card-title mb-0 text-white">
                        ...
                        {{ $isEdit ? 'Edit' : 'Process' }} Arrear Management
                    </h5>
                </div>
                <div class="card-body p-4">
                    <form action="{{ $isEdit ? route('arrear.update', $arrearData->id) : route('arrear.store') }}"
                          method="POST" id="arrearForm">
                        @csrf
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="select2_list" name="company_id" id="company_id" required>
                                    ...
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}" {{ ($isEdit && $arrearData->company_id == $company->id) ? 'selected' : '' }}>
                                            {{ $company->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-4 g-3">
                                ...
                                <select class="select2_list" name="pay_group_id" id="pay_group_id" required>
                                    ...
                                    @foreach($payGroups as $group)
                                        <option value="{{ $group->id }}" 
                                                ...
                                                {{ ($isEdit && $arrearData->pay_group_id == $group->id) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4" id="salary_month_container">
                                <label class="form-label fw-semibold">Arrear Month <span class="text-danger">*</span></label>
                                <input type="month" class="form-control" name="salary_month" id="salary_month" 
                                       value="{{ $isEdit ? $arrearData->salary_month : date('Y-m') }}">
                            </div>

                            <div class="col-md-4 custom-date-container d-none">
                                <label class="form-label fw-semibold">Start Date <span class="text-danger">*</span></label>
                                <input type="date" class="form-control" name="start_date" id="start_date" 
                                       value="{{ $isEdit ? \Carbon\Carbon::parse($arrearData->start_date)->format('Y-m-d') : '' }}">
                            </div>

                            <div class="col-md-4 custom-date-container d-none">
                                <label class="form-label fw-semibold">End Date <span class="text-danger">*</span></label>
                                <input type="date" class="form-control" name="end_date" id="end_date" 
                                       value="{{ $isEdit ? \Carbon\Carbon::parse($arrearData->end_date)->format('Y-m-d') : '' }}">
                            </div>

                            <div class="col-md-4">
                                <label class="form-label fw-semibold">Target Payment Month <span class="text-danger">*</span></label>
                                <input type="month" class="form-control" name="payment_month" id="payment_month" 
                                       value="{{ isset($firstItem) ? $firstItem->payment_month : date('Y-m') }}" required>
                            </div>
                        </div>
                        ...
                        <div class="row g-4 mb-4">
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" name="reason" rows="2" placeholder="Describe the reason for this arrear payment">{{ isset($firstItem) ? $firstItem->reason : '' }}</textarea>
                            </div>
                        </div>

                        <div class="mt-4 pt-3 border-top text-end">
                            <a href="{{ route('arrear.index') }}" class="btn btn-light px-4 me-2">Cancel</a>
    ...
    <script>
        $(function() {
            ...
            @if($isEdit)
            const editData = {
                company: "{{ $arrearData->company_id ?? '' }}",
                branch: "{{ $arrearData->branch_id ?? '' }}",
                division: "{{ $arrearData->division_id ?? '' }}",
                department: "{{ $arrearData->department_id ?? '' }}",
                section: "{{ $arrearData->section_id ?? '' }}",
