...
<head>
    ...
    <title>Employee Increment Logs - Print</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 15mm 10mm;
...
<body>
    ...
    <div class="report-header">
        <h1>Employee Salary Increment Sheet</h1>
        <p>List of all employee salary increment history, methods, and adjustments</p>
        <p><strong>Report Date:</strong> {{ date('l, F j, Y') }}</p>
    </div>

    <div class="report-info">
        <div><strong>Total Increments:</strong> {{ count($records) }}</div>
        @if(request('keyword'))
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width:12%;">Increment Base</th>
                <th style="width:12%;">Increment Method</th>
                <th style="width:12%;">Increase Amount</th>
                <th style="width:12%;">Prev Gross Salary</th>
                <th style="width:12%;">New Gross Salary</th>
        ...
        <tbody>
            @foreach ($records as $index => $record)
                <tr>
                    ...
                    <td>{{ ucwords(str_replace('_', ' ', $record->increment_base ?? 'N/A')) }}</td>
                    <td>{{ ucfirst($record->increment_method ?? 'N/A') }}</td>
                    <td>৳{{ number_format($record->salary_increase_amount ?? 0, 2) }}</td>
