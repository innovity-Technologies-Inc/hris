...
<head>
    ...
    <title>Employee Decrement Logs - Print</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 15mm 10mm;
...
<body>
    ...
    <div class="report-header">
        <h1>Employee Salary Decrement Sheet</h1>
        <p>List of all employee salary decrement history, methods, and adjustments</p>
        <p><strong>Report Date:</strong> {{ date('l, F j, Y') }}</p>
    </div>

    <div class="report-info">
        <div><strong>Total Decrements:</strong> {{ count($records) }}</div>
        @if(request('keyword'))
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width:12%;">Decrement Base</th>
                <th style="width:12%;">Decrement Method</th>
                <th style="width:12%;">Decrease Amount</th>
        ...
        <tbody>
            @foreach ($records as $index => $record)
                <tr>
                    ...
                    <td>{{ ucwords(str_replace('_', ' ', $record->decrement_base ?? 'N/A')) }}</td>
                    <td>{{ ucfirst($record->decrement_method ?? 'N/A') }}</td>
