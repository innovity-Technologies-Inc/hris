...
@section('content')
    <div class="row mb-3">
        <div class="col-12">
            <a href="{{ route('decrement.index') }}" class="btn btn-outline-secondary btn-sm">
                <i style="height: 12px; width: 12px" data-feather="arrow-left"></i> Back to Decrements
            </a>
        </div>
    </div>
    ...
    <form id="employeeDecrementForm" method="POST"
        action="{{ isset($decrementData) ? route('decrement.update', $decrementData->id) : route('decrement.store') }}"
                                ...
                                <select name="employee_id" id="employee_id"
                                    ...
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}"
                                            ...
                                            {{ old('employee_id', $decrementData->employee_id ?? '') == $employee->id ? 'selected' : '' }}>
        ...
        {{-- Decrement Details Section --}}
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0">Decrement Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            {{-- Hidden fields for previous salary information --}}
                            <input type="hidden" name="previous_basic_salary" id="previous_basic_salary"
                                value="{{ old('previous_basic_salary', $decrementData->previous_basic_salary ?? '') }}">
                            <input type="hidden" name="previous_gross_salary" id="previous_gross_salary"
                                value="{{ old('previous_gross_salary', $decrementData->previous_gross_salary ?? '') }}">
                                ...
                                <select name="pay_scale_id" id="pay_scale_id"
                                    ...
                                    @foreach ($payScales as $scale)
                                        <option value="{{ $scale->id }}"
                                            ...
                                            {{ old('pay_scale_id', $decrementData->pay_scale_id ?? '') == $scale->id ? 'selected' : '' }}>
                                ...
                                <select name="movement_type_id" id="movement_type_id"
                                    ...
                                    @foreach ($movementTypes as $type)
                                        <option value="{{ $type->id }}"
                                            {{ old('movement_type_id', $decrementData->movement_type_id ?? '') == $type->id ? 'selected' : '' }}>
                                            {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            {{-- Decrement Base --}}
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Decrement Base <span class="text-danger">*</span></label>
                                <select name="decrement_base" id="decrement_base"
                                    ...
                                    <option value="basic_salary"
                                        {{ old('decrement_base', $decrementData->decrement_base ?? '') == 'basic_salary' ? 'selected' : '' }}>
                                        Basic Salary
                                    </option>
                                    <option value="gross_salary"
                                        {{ old('decrement_base', $decrementData->decrement_base ?? '') == 'gross_salary' ? 'selected' : '' }}>
                                        Gross Salary
                                    </option>
                                </select>
                            ...
                            {{-- Decrement Method --}}
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Decrement Method <span class="text-danger">*</span></label>
                                <select name="decrement_method" id="decrement_method"
                                    ...
                                    <option value="fixed"
                                        {{ old('decrement_method', $decrementData->decrement_method ?? '') == 'fixed' ? 'selected' : '' }}>
                                        Fixed Amount
                                    </option>
                                    <option value="percentage"
                                        {{ old('decrement_method', $decrementData->decrement_method ?? '') == 'percentage' ? 'selected' : '' }}>
                                        Percentage
                                    </option>
                                </select>
                            ...
                            {{-- Decrement Amount --}}
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Decrement Amount <span class="text-danger">*</span></label>
                                <input type="number" name="salary_decrease_amount" id="salary_decrease_amount"
                                    value="{{ old('salary_decrease_amount', $decrementData->salary_decrease_amount ?? '') }}"
                                    class="form-control @error('salary_decrease_amount') is-invalid @enderror" step="0.01"
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="effective_from" id="effective_from"
                                    value="{{ old('effective_from', isset($decrementData) ? $decrementData->effective_from : '') }}"
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="effective_to" id="effective_to"
                                    value="{{ old('effective_to', isset($decrementData) && $decrementData->effective_to ? $decrementData->effective_to : '') }}"
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                @if(isset($decrementData) && $decrementData->attachments->count() > 0)
                                    <div class="mt-2">
                                        <small class="text-muted d-block mb-1">Current Attachments:</small>
                                        <div class="d-flex flex-wrap gap-2">
                                            @foreach($decrementData->attachments as $attachment)
                            ...
                            <div class="col-md-12">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('decrement.index') }}" class="btn btn-secondary">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#employeeDecrementForm').on('submit', function(e) {
                e.preventDefault();
            ...
            $('#employee_id').on('change', function() {
                ...
                if (salaryBreakdown) {
                    ...
                    // Pre-select employee's current pay scale in create mode
                    @if(!isset($decrementData))
                        if (salaryBreakdown.pay_scale_id) {
                            $('#pay_scale_id').val(salaryBreakdown.pay_scale_id).trigger('change');
                        } else {
            ...
            // 3. Pay Scale Selection & Verification (Subtraction logic)
            ...
            function verifyPayScale() {
                ...
                if (selectedOption.val()) {
                    ...
                    if (max > 0 && newGrossSalary > max) {
                        const warningMsg = `Warning: The decremented salary (${formatCurrency(newGrossSalary)}) surpasses the selected pay scale maximum limit of (${formatCurrency(max)}).`;
                        ...
                        const warningMsg = `Warning: The decremented salary (${formatCurrency(newGrossSalary)}) is below the selected pay scale minimum limit of (${formatCurrency(min)}).`;
                        $('#pay_scale_id').after(`<div id="payscale-warning" class="alert alert-warning py-1 px-2 mt-2 mb-0 small text-danger"><i class="bi bi-exclamation-triangle"></i> ${warningMsg}</div>`);
                    }
                }
            }
