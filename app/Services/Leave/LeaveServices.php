...
class LeaveServices
{
    ...
    /**
     * Get all leaves for export/print without pagination.
     */
    public function getLeavesAll(FlexSearch $flexsearch, Request $request)
    {
        $query = Leave::with('getEmployee.officeInfo.getCurrentCompany', 'getEmployee.officeInfo.getCurrentBusinessUnit', 'getPlan');
        $searchableColumns = ['getEmployee.full_name', 'getPlan.name', 'getPlan.leave_type', 'leave_count'];
        $keyword = $request->input('keyword');
        ...
        return $flexsearch->apply($query, $filters, $keyword, $searchableColumns)->get();
    }
