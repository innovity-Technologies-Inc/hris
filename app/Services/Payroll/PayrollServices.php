    ...
    protected function handleAttachments($model, $folder)
    {
        if (request()->hasFile('attachments')) {
            foreach (request()->file('attachments') as $file) {
                $path = \App\HelperClass::file_upload($file, $folder, false);
            ...
            foreach ($employees as $employee) {
                ...
                if ($taxCalculation && $isTaxApplicable) {
                    if ($frequency === 'weekly') {
                        ...
                        $dailyTax = (double) $taxCalculation->tax_per_month / $workingDaysPerCycle;
                        $taxDeduction = $dailyTax * $totalDaysInRange;
                    } elseif ($frequency === 'hourly') {
                        $hourlyTax = (double) $taxCalculation->tax_per_month / ($workingDaysPerCycle * $workingHoursPerDay);
                        $hoursWorked = $totalShiftMinutes / 60;
                        $taxDeduction = $hourlyTax * $hoursWorked;
                    } else {
