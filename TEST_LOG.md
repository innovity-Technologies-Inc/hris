...
**Goal**: Implement API-first and Axios-based travel movement submissions with FormRequests and service classes. Drop deprecated single-route columns (`source_address`, `destination_address`, etc.) and custom allowance columns (`custom_ta`, `custom_da`) from the database. Support auto-populating new leg source address with the previous leg's destination, and new leg destination address with the previous leg's source in JS (return journey auto-population). Enable directly editable total TA and DA fields without selecting plans. Rename "Route Leg" to "Route" and change borders to use the project's primary color.
...
**Results**:
- Modified JS inside `form.blade.php` to extract both source and destination from the last card, auto-populating the return trip (new source = old destination, new destination = old source) and triggering immediate distance calculations.
