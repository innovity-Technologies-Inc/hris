...
beforeEach(function () {
    ...
    Permission::firstOrCreate(['name' => 'arrear.view', 'guard_name' => 'web']);
    Permission::firstOrCreate(['name' => 'arrear.create', 'guard_name' => 'web']);
    Permission::firstOrCreate(['name' => 'arrear.edit', 'guard_name' => 'web']);
    Permission::firstOrCreate(['name' => 'arrear.delete', 'guard_name' => 'web']);
...
test('it exports and prints arrear process lists and batch details', function () {
    $this->withoutExceptionHandling();

    $hrUser = User::factory()->create(['user_type' => UserType::Company, 'name' => 'HR Manager']);
    $hrUser->givePermissionTo('arrear.view');
    $hrUser->givePermissionTo('arrear.create');
    ...
    // Create an arrear batch
    $response = $this->actingAs($hrUser)->post(route('arrear.store'), [
    ...
    expect($process->type)->toBe('arrear');

    // Test export and print routes
    $this->withoutMiddleware();

    // Index-level export and print
    $response = $this->actingAs($hrUser)->get(route('arrear.export.excel'));
    $response->assertStatus(200);
    $this->assertTrue(str_contains($response->headers->get('content-disposition'), 'attachment; filename=arrear_adjustment_processes_'));

    $response = $this->actingAs($hrUser)->get(route('arrear.print'));
    $response->assertStatus(200);
    $response->assertViewIs('payroll.arrear.print_index');

    // Process-level export and print
    $response = $this->actingAs($hrUser)->get(route('arrear.process.export.excel', $process->id));
    $response->assertStatus(200);
    $this->assertTrue(str_contains($response->headers->get('content-disposition'), 'attachment; filename=arrear_adjustments_'));

    $response = $this->actingAs($hrUser)->get(route('arrear.process.print', $process->id));
    $response->assertStatus(200);
    $response->assertViewIs('payroll.arrear.print_process');
});
