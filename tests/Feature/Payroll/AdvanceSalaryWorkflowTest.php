...
test('it processes advance salary using central approval engine sequential workflow', function () {
    $this->withoutExceptionHandling();

    $hrRole = Role::firstOrCreate(['name' => 'HR Manager', 'guard_name' => 'web']);
    
    $workflow = Workflow::create([
        'name' => 'Advance Salary Approval Workflow',
    ...
    $admin->givePermissionTo('advance-salary.create');

    $response = $this->actingAs($admin)->postJson(route('advance-salary.store'), [
    ...
    expect($process->total_amount)->toEqual(5000.00);

    $item = AdvanceSalary::latest()->first();
    ...
    $response = $this->actingAs($hrUser)->postJson(route('approval.action', $stepRequest->id), [
        'action' => 'approve',
        'comments' => 'Approve advance salary batch'
    ]);
