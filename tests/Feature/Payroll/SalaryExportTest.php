...
test('it exports and prints salary process lists and batch details', function () {
    ...
    // Create a salary process
    $response = $this->actingAs($hrUser)->post(route('salary.store'), [
    ...
    expect($process->type)->toBe('salary');

    // Test export and print routes
    $this->withoutMiddleware();

    // Index-level export and print
    $response = $this->actingAs($hrUser)->get(route('salary.export.excel'));
    $response->assertStatus(200);
    $this->assertTrue(str_contains($response->headers->get('content-disposition'), 'attachment; filename=salary_generation_processes_'));

    $response = $this->actingAs($hrUser)->get(route('salary.print'));
    $response->assertStatus(200);
    $response->assertViewIs('payroll.salary.print_index');

    // Process-level export and print
    $response = $this->actingAs($hrUser)->get(route('salary.process.export.excel', $process->id));
    $response->assertStatus(200);
    $this->assertTrue(str_contains($response->headers->get('content-disposition'), 'attachment; filename=salary_calculations_'));

    $response = $this->actingAs($hrUser)->get(route('salary.process.print', $process->id));
    $response->assertStatus(200);
    $response->assertViewIs('payroll.salary.print_process');
});
